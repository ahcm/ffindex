...
int ffindex_apply_by_entry(char *data, ffindex_index_t* index, ffindex_entry_t* entry, char* program_name, char** program_argv, FILE* data_file_out, FILE* index_file_out, size_t *offset, int quiet)
{
  int ret = 0;
...
int main(int argn, char **argv)
{
  ...
  if(argn - optind < 3)
  {
    ...
    fprintf(stderr, "USAGE: %s [-m] [-q] -d DATA_FILENAME_OUT -i INDEX_FILENAME_OUT DATA_FILENAME INDEX_FILENAME -- PROGRAM [PROGRAM_ARGS]*\n"
                    ...
                    "\t-q\tquiet, do not print entry processing information\n"
  ...
  if(batch_size > 0)
    for(size_t entry_index = range_start; entry_index < range_end; entry_index++)
    {
      ...
      int error = ffindex_apply_by_entry(data, index, entry, program_name, program_argv, data_file_out, index_file_out, &offset, quiet);
      if(error != 0)
        { perror(entry->name); break; }
    }
  ...
  if(merge && data_filename_out != NULL && mpi_rank == 0)
  {
    ...
    {
      snprintf( merge_command, FILENAME_MAX, "%s -as %s %s -d %s.%d -i %s.%d",
                argv[0], data_filename_out, index_filename_out, data_filename_out, i, index_filename_out, i);
      //puts(merge_command);
